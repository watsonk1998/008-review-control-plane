...
- `construction_scheme` 当前是 experimental documentType，其 ready base pack 已可与 `lifting_operations / temporary_power / hot_work` 场景 pack 组合运行
- `supervision_plan` 与 `review_support_material` 当前也是 experimental documentType，仍只启用各自 base pack
...
- eval CLI JSON 会新增 `layeredMetrics`
- `L0 / L1 / L2 / CrossCutting` 只是对现有 aggregate 的分组视图
- `L3` 目前只保留 `diagnosticOnly: true`，不伪造新评分
