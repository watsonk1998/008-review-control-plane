...
以下类型当前已有可执行 `ready` base packs，但 documentType 当前只处于 `experimental` 范围，不作为 P1 主支持对象：
...
- 当前 documentType readiness 固定区分为 `official / experimental / skeleton`
...
- `structured-review-l0-visibility.json` 只复写 canonical L0 事实：`parseMode / parserLimited / parseWarnings / visibility / manualReviewNeeded / attachments`
- reviewer/UI 继续以 `result.visibility` 为唯一 canonical contract；L0 artifact 仅用于下载、debug 与回归
