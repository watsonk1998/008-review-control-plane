...
> **本文档职责**
> - 负责：
>   - 说明 008 的系统架构、模块边界、主要组件关系与整体技术组织方式
> - 不负责：
>   - 不定义产品边界、验收标准、官方能力口径
>   - 不替代实施任务书
> - 主适用读者：
>   - 架构师、研发工程师、核心协作者
> - 冲突处理：
>   - 涉及产品定义与治理口径时，以 product / governance 层文档为准
> - 文档状态：
>   - 设计说明 / 主真相源
...
- `construction_scheme / supervision_plan / review_support_material` 当前都属于 experimental documentType，而不是 official
- `gas_area_ops.base` 当前只对 `construction_org` 与 `hazardous_special_scheme` 生效

evidence packs 继续用 Python/Pydantic registry 管理，并同步生成 `fixtures/construction/derived/evidence_packs/*.json`，不进入 YAML/DSL 平台化。
...
- `L3`：diagnostic-only remediation / suggestion separation
