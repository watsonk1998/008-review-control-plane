...
## Project Corrections Addendum (2026-04-16, 夜间批次 — Kilo Code Worktree 治理 + CSS 清洗深化)

> Source: Kilo Code Agent Manager 的 worktree patch-apply 机制在 hermes-review-agent 上引发不可恢复的脏合并状态，导致 git 操作全面阻塞。同会话完成 provenance 重构部分合并、weknora 部署及根因定位。

### Kilo Code Worktree 硬门禁（HG-30）

- **Kilo Code 的"立即应用"（Apply Now）禁止用于 5+ 文件的大规模变更**：该按钮通过 patch-apply（非 git merge）将 worktree diff 打到主工作目录。对于大范围修改（如 10 个文件 +2618/-1465），极高概率触发三方合并冲突，且冲突后 Kilo 不会执行 cleanup——脏的 merge index（`UU` 状态）会阻塞一切 git 操作（checkout、commit、stash），**只有 `git merge --abort` 能解除**，`git reset --hard` 无效。
- **Kilo worktree 的 patch 可能是破坏性的**：本次事故中 `sqlite_store.py` 从 558 行被截断至 320 行，所有 CRUD 方法（`get_task`、`create_task`、`update_task`、`list_tasks`、`_row_to_task` 等）全部丢失。AI Agent 的大规模 patch 不可信——接受前必须验证不含截断。
- **Kilo worktree 的修改从不 commit**：worktree 中的所有改动以 unstaged modifications 存在。Agent Manager 面板上显示的 `+N -M` 是永远不会自动消失的脏状态，除非手动进入 worktree commit 或删除 worktree。
- **脏合并状态修复 SOP**：`git merge --abort 2>/dev/null; git cherry-pick --abort 2>/dev/null; git rebase --abort 2>/dev/null; git reset --hard HEAD`。四条命令必须全部执行，只执行 `reset --hard` 不够。
- **worktree 清理命令**：`git worktree remove .kilo/worktrees/<name> --force && git branch -D <branch-name>`。
- **推荐做法**：对 hermes-review-agent 这种大型项目，关闭 Kilo 的 worktree 模式，让 Kilo 直接在当前分支上工作。若必须使用 worktree，先在 worktree 内 commit，再用 `git merge --no-ff` 合并。

### 前端 CSS 清洗深度合同（HG-31，强化 HG-25）

- **`StructuredReportHtml` 的 CSS 清洗必须包含 5 条规则**（与上方 HG-25 的 2 条版本冲突时以本条为准）：
  ```
  ...
  ```
- 前 2 条防御打印强制重排（HG-25 原始规则），后 3 条防御滚动白屏和 hover 卡顿（2026-04-16 下午批次新增）。
- 任何合并冲突时必须保留完整 5 条版本，不得回退到 2 条简化版。
