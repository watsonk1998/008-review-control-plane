...
const moduleBindings = JSON.parse(
  fs.readFileSync(path.join(generatedDir, 'module_bindings.json'), 'utf8')
);
const docTypes = Object.keys(
  JSON.parse(fs.readFileSync(path.join(generatedDir, 'profile_mapping.json'), 'utf8'))
);
...
md.push(
  ...
  '## Import readiness',
  '',
  '- Workflow JSON generated into `/artifacts/fastgpt`',
  '- Governed assets exported from the Python source of truth',
  '- Toolset source is present under `/integrations/fastgpt/toolset/hermesStructuredReview`',
  '- Official `.pkg` build requires Bun + temporary clone of `labring/fastgpt-plugin`'
);
