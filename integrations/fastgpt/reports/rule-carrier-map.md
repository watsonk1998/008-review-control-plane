...
- 主工作流只负责输入归一化、文件 URL 显式传递、5 个工作流工具编排、正式/降级出口。
...
| `hermes_ai_review_wft` | `prepareAiReview` | 按 `enabledModules`、`documentType`、模板 `supported_document_types` 选择 reviewer | code 常量 + template manifest |
| `hermes_ai_review_wft` | `runAiReview` | reviewer prompt 与结构化 JSON 主审要求 | prompt |
...
1. `hermes_ai_review_wft/runAiReview` 的 reviewer prompt 与输出 JSON 要求。
2. 主工作流变量默认值：`documentType`、`enabledModules`、`strictMode`。
3. 必要时调整 `focusRequirements`，但不得绕过 `hermes_review_context_wft` 的 profile/basis 解析。

禁止在主工作流里直接新增 `documentType -> basis file path` 或把大段规范原文塞进 prompt。依据选择必须走 governance 快照或后续配置工具。

### v2：平台可维护配置工具

建议新增一个清晰命名的“规则配置/模板配置”工作流工具或配置节点，把以下内容从大 code 常量中拆出：
...
- key identity probe 命中 `normalizeInput -> callReviewContextWft -> callAiReviewWft -> callFinalAssemblerWft -> formalReply/degradedReply`。
