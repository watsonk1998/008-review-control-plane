...
def extract_hazard_facts(
    ...
    for block in parse_result.blocks:
        ...
        if (
            "煤气区域" in content or "煤气" in content
        ) and "gas_area_ops" not in risk_categories:
            risk_categories.append("gas_area_ops")
            block_refs["hazard.highRiskCategories"].append(block["id"])
        if (
            any(keyword in content for keyword in ["施工用电", "临时用电", "停送电"])
            and "temporary_power" not in risk_categories
        ):
        ...
        if any(
            keyword in content
            for keyword in ["安全保证措施", "安全技术措施", "安全管理措施"]
        ):
    ...
    dedicated_section = any(
        "专项施工方案" in section["title"] and int(section.get("level", 99)) <= 2
    ...
    facts = {
        ...
        "calculatedLiftWeightTon": float(weight_match.group(1))
        if weight_match
        else None,
        "specialSchemePlanStatus": special_scheme_status,
        "calculationEvidencePresent": bool(
            block_refs["hazard.calculationEvidencePresent"]
        )
        or bool(crane_match and weight_match),
        ...
        "measureKeywordCount": sum(
            1 for keyword in _MEASURE_KEYWORDS if keyword in text
        ),
    }

    # Generate span_ids for any blocks that will become EvidenceSpans
    span_id_map = {}
    for fact_key, block_ids in block_refs.items():
        for block_id in block_ids:
            span_id_map[block_id] = hashlib.md5(
                f"{fact_key}{block_id}".encode()
            ).hexdigest()[:16]
    unresolved = []
