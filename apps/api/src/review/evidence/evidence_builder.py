...
from src.domain.models import FindingType, ReviewLayer
...
class EvidenceBuilder:
    ...
    def build(self, rule_hits: list[RuleHit], facts: ExtractedFacts, parse_result, packs) -> list[IssueCandidate]:
        ...
        for hit in rule_hits:
            ...
            policy_evidence = self.clause_store.get_policy_evidence(hit.ruleId)
    ...
    def _collect_doc_evidence(self, fact_refs: list[str], facts: ExtractedFacts):
        ...
        for fact_ref in fact_refs:
            for span in facts.factEvidence.get(fact_ref, []):
                key = (span.sourceId, tuple(sorted(span.locator.items())) if isinstance(span.locator, dict) else str(span.locator), span.excerpt)
                if key in seen:
                    continue
                seen.add(key)
