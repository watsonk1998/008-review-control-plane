...
class EvidenceBuilder:
    def __init__(self, clause_store: ClauseStore | None = None):
        ...
        self._titles = {
            ...
            "distribution_network_special_scheme_risk_identification": "配网工程专项缺少稳定的风险辨识与分级表达",
            "distribution_network_special_scheme_drawings_and_boundary": "配网工程专项图纸或作业边界信息需补强",
            "distribution_network_special_scheme_emergency_targeted": "配网工程专项应急处置安排针对性不足",
            "power_outage_work_structure_completeness": "停电施工作业专项章节不完整",
            "power_outage_work_safety_and_quality_controls": "停电施工作业安全/质量控制链不完整",
            "power_outage_work_ticket_grounding_traceability": "停电施工作业工作票/验电接地证据链不可追溯",
            "power_outage_work_basic_info_integrity": "停电施工基础信息及编制依据不完整",
            "power_outage_work_personnel_qualification_and_training": "停电施工人员资质、培训与监护安排不足",
            "power_outage_work_application_approval_linkage": "停电申请审批与用户告知闭环不完整",
            "power_outage_work_shutdown_five_step_closure": "停电五步法安全措施闭环不完整",
            "power_outage_work_anti_backfeed_controls": "防反送电与双电源防控措施不足",
            "power_outage_work_work_ticket_and_site_survey": "工作票/操作票/现场勘察证据链不足",
            "power_outage_work_restoration_and_archive_closure": "完工送电、资料归档与整改闭环不完整",
        ...
        self._finding_types = {
            ...
            "distribution_network_special_scheme_risk_identification": FindingType.hard_evidence,
            "distribution_network_special_scheme_drawings_and_boundary": FindingType.visibility_gap,
            "distribution_network_special_scheme_emergency_targeted": FindingType.hard_evidence,
            "power_outage_work_structure_completeness": FindingType.hard_evidence,
            "power_outage_work_safety_and_quality_controls": FindingType.hard_evidence,
            "power_outage_work_ticket_grounding_traceability": FindingType.engineering_inference,
            "power_outage_work_basic_info_integrity": FindingType.hard_evidence,
            "power_outage_work_personnel_qualification_and_training": FindingType.hard_evidence,
            "power_outage_work_application_approval_linkage": FindingType.hard_evidence,
            "power_outage_work_shutdown_five_step_closure": FindingType.hard_evidence,
            "power_outage_work_anti_backfeed_controls": FindingType.hard_evidence,
            "power_outage_work_work_ticket_and_site_survey": FindingType.engineering_inference,
            "power_outage_work_restoration_and_archive_closure": FindingType.engineering_inference,
        ...
        self._closed_negative_fact_prefixes = (
            "project.sectionPresence.",
            "project.structureCompleteness.",
        )
    ...
    def build(
        self, rule_hits: list[RuleHit], facts: ExtractedFacts, parse_result, packs
    ) -> list[IssueCandidate]:
        ...
        for hit in rule_hits:
            ...
            policy_evidence = self.clause_store.get_policy_evidence(
                hit.ruleId, selected_pack_ids
            )
            base_finding_type = self._finding_types.get(
                hit.ruleId, FindingType.hard_evidence
            )
            visibility_gap = hit.applicabilityState == "blocked_by_visibility" or any(
                span.visibility and span.visibility.value != "parsed"
                for span in doc_evidence
            )
            manual_review_needed = (
                hit.status == "manual_review_needed"
                or hit.applicabilityState
                in {
            ...
            doc_evidence = self._annotate_evidence_spans(
                doc_evidence, gap_reason=gap_reason
            )
            policy_evidence = self._annotate_evidence_spans(
                policy_evidence, gap_reason=gap_reason
            )
            finding_type = (
                FindingType.visibility_gap
                if hit.applicabilityState == "blocked_by_visibility"
                else base_finding_type
            )
            candidates.append(
                IssueCandidate(
    ...
    def _resolve_manual_review_reason(
        ...
        *,
        ...
        if applicability_state == "blocked_by_visibility":
            reason = self._resolve_visibility_reason(
                blocking_reasons=blocking_reasons, doc_evidence=doc_evidence
            )
            if reason:
                return reason
        visibilities = [
            span.visibility.value
            for span in doc_evidence
            if span.visibility is not None
        ]
    ...
    def _resolve_visibility_reason(
        self, *, blocking_reasons: list[str], doc_evidence
    ) -> str | None:
        ...
        if (
            "unknown" in visibilities
            or {"visibility_unknown", "attachment_unknown"} & reasons
        ):
            return "visibility_unknown"
    ...
    def _is_closed_negative_hit(self, hit: RuleHit) -> bool:
        ...
        if not required_fact_keys:
            return False
        return all(
            self._is_closed_negative_fact_key(fact_key)
            for fact_key in required_fact_keys
        )

    def _is_closed_negative_fact_key(self, fact_key: str) -> bool:
        return (
            fact_key.startswith(self._closed_negative_fact_prefixes)
            or fact_key in self._closed_negative_fact_keys
        )
    ...
    def _annotate_evidence_spans(self, spans, *, gap_reason: str | None):
        ...
        for span in spans:
            updated = span.model_copy(
                update={"evidenceGapReason": span.evidenceGapReason or gap_reason}
            )
            # Generate or use existing span_id for provenance tracking
            if not updated.span_id:
                import hashlib

                span_hash = hashlib.md5(
                    f"{span.sourceId}{span.locator}{span.excerpt}".encode()
                ).hexdigest()[:16]
                updated = updated.model_copy(update={"span_id": span_hash})
    ...
    def _derive_gap_reason(
        ...
        *,
        ...
        if applicability_state == "blocked_by_visibility":
            return self._resolve_visibility_reason(
                blocking_reasons=blocking_reasons, doc_evidence=[]
            )
