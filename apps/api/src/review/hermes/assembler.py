...
class HermesReviewAssembler:
    ...
    def assemble(
        ...
        *,
        ...
        provenance_registry: dict[str, Any] | None = None,
    ) -> tuple[dict[str, Any], FinalReportPacket | None]:

        hermes_ok = bool(
            hermes_review_packets and not all(p.degraded for p in hermes_review_packets)
        )
        ...
        if not hermes_ok:
            ...
            payload = dict(
                support_result_008
                or (support_packet_008.raw_result if support_packet_008 else {})
                or {}
            )
            ...
            fallback_markdown = self._render_degraded_markdown(
                doc_label=_GRADE_LABELS.get(
                    str(brief.review_object_type), str(brief.review_object_type)
                ),
        ...
        if enabled_modules:
            ...
            if blocked_modules:
                ...
                blocked_titles = "、".join(
                    block_details[m]["title"] for m in blocked_modules
                )
                error_reason = (
                    "；".join(error_parts)
                    if error_parts
                ...
                payload["hermesController"] = {
                    "enabled": True,
                    "selectedAgents": [
                        result.get("agent_id") for result in agent_results
                    ],
                    "agentResults": agent_results,
                    "finalReportReady": False,
                    "supplementalPacketCount": 0,
        ...
        merged_hermes_main_review = self._merge_hermes_review_outcomes(
            support_packet_008.review_id if support_packet_008 else brief.review_id,
            hermes_review_packets,
        )
        support_packet_008, merged_hermes_main_review = (
            self._apply_support_confidence_adjustments(
                support_packet_008=support_packet_008,
                hermes_review_packet=merged_hermes_main_review,
            )
        )
        support_packet_008, merged_hermes_main_review = (
            self._resolve_conflicts_and_gaps(
                support_packet_008=support_packet_008,
                hermes_review_packet=merged_hermes_main_review,
            )
        )
        support_packet_008 = self._filter_packet_by_enabled_modules(
            support_packet_008, enabled_modules
        )
        merged_hermes_main_review = self._filter_packet_by_enabled_modules(
            merged_hermes_main_review, enabled_modules
        )

        # Extract provenance registry from support packet if not explicitly provided
        if not provenance_registry and support_packet_008:
            provenance_registry = support_packet_008.metadata.get(
                "provenance_registry"
            ) or support_packet_008.raw_result.get("provenance_registry")

        # Apply provenance validation
        if provenance_registry:
            if support_packet_008:
                support_packet_008 = support_packet_008.model_copy(deep=True)
                support_packet_008.findings = self._validate_findings_provenance(
                    support_packet_008.findings, provenance_registry
                )
            if merged_hermes_main_review:
                merged_hermes_main_review = merged_hermes_main_review.model_copy(
                    deep=True
                )
                merged_hermes_main_review.findings = self._validate_findings_provenance(
                    merged_hermes_main_review.findings, provenance_registry
                )
    ...
    def _render_degraded_markdown(
        self, doc_label: str, support_packet: FactPacket | None, error_reason: str
    ) -> str:
        lines: list[str] = []
    ...
    def _merge_hermes_review_outcomes(
        self, review_id: str, packets: list[FactPacket]
    ) -> FactPacket | None:
        if not packets:
            return None
        findings = []
        ...
        metrics = ReviewPacketMetrics(
            ...
            grounded_findings=sum(
                1 for item in findings if item.evidence_status == "grounded"
            ),
            evidence_gap_findings=sum(
                1 for item in findings if item.evidence_status == "evidence_gap"
            ),
            manual_review_needed=sum(
                1 for item in findings if item.manual_review_needed
            ),
        )
        return FactPacket(
            review_id=review_id,
            ...
            findings=findings,
            overall_assessment="；".join(
                packet.overall_assessment
                for packet in packets
                if packet.overall_assessment
            ),
        ...
        *,
        ...
        for finding in adjusted_support.findings:
            if finding.title in hermes_titles:
                finding.raw_data = {
                    **finding.raw_data,
                    "hermes_decision_signal": "corroborated",
                }
        ...
        *,
        ...
        for finding in adjusted_hermes.findings:
            if finding.evidence_status in {"evidence_gap", "visibility_gap"}:
                finding.raw_data = {
                    **finding.raw_data,
                    "decision_action": "degraded_by_support_evidence_gap",
                }
            elif finding.title not in support_titles:
                finding.raw_data = {
                    **finding.raw_data,
                    "decision_action": "supplement_from_support_gap_scan",
                }
    ...
    def _build_final_decision_packet(
        ...
        *,
        ...
        top_risks = [
            ...
            if item.severity == "high" or item.hallucination_risk
        ]
    ...
    def _filter_packet_by_enabled_modules(
        ...
        filtered.summary_metrics = ReviewPacketMetrics(
            ...
            high_severity=sum(
                1 for finding in filtered.findings if finding.severity == "high"
            ),
            medium_severity=sum(
                1 for finding in filtered.findings if finding.severity == "medium"
            ),
            low_severity=sum(
                1 for finding in filtered.findings if finding.severity == "low"
            ),
        )
        filtered.metadata = {
            **filtered.metadata,
    ...
    def _finding_in_enabled_modules(
        self, finding: FindingItem, allowed: set[str]
    ) -> bool:
        ...
        return any(
            isinstance(module, str) and module in allowed for module in review_modules
        )

    def _packet_review_modules(
        self, packet: FactPacket, allowed: set[str] | None = None
    ) -> list[str]:
        modules: list[str] = []
    ...
    def _decide_grade(self, all_findings: list[FindingItem]) -> str:
        high = sum(
            ...
            if finding.severity == "high" or finding.hallucination_risk
        )
    ...
    def _render_markdown(
        ...
        for dim_title, items in grouped_findings.items():
            ...
            for finding in items:
                severity_zh = severity_zh_map.get(
                    finding.severity.lower(), finding.severity.upper()
                )
        ...
        for engine, info in degradation.items():
            lines.append(
                f"- 组件降级通报: {engine} 模块异常 ({info.get('reason', '未知')})"
            )
        lines.append("")
        return "\n".join(lines)
    ...
    def _annotate_final_decision_findings(
        ...
        *,
        ...
        hermes_modules = list(
            (hermes_review_packet.metadata if hermes_review_packet else {}).get(
                "review_modules"
            )
            or []
        )
        ...
        for finding in packet.key_findings:
            ...
            finding.raw_data = {
                **finding.raw_data,
                ...
                "decision_owner": "hermes",
                "ownership": finding.raw_data.get(
                    "ownership",
                    support_packet_008.metadata.get("ownership", "support_material"),
                ),
                "source_template_id": finding.raw_data.get("template_id")
                or support_packet_008.metadata.get("template_id"),
                "review_modules": finding.raw_data.get("review_modules")
                or support_modules,
        ...
        for finding in packet.supplemental_findings:
            ...
            finding.raw_data = {
                **finding.raw_data,
                ...
                "source_template_id": finding.raw_data.get("template_id")
                or (
                    hermes_review_packet.metadata.get("template_id")
                    if hermes_review_packet
                    else None
                ),
                "review_modules": finding.raw_data.get("review_modules")
                or hermes_modules,
        ...
        final_index = {
            finding.id: finding
            for finding in packet.key_findings + packet.supplemental_findings
        }
    ...
    def _extract_error_reason(self, hermes_review_packets: list[FactPacket]) -> str:
        ...
        reasons = [
            (
                p.metadata.get("template_id")
                or p.metadata.get("agent_id")
                or "unknown",
                p.error or "",
            )
    ...
    def _validate_findings_provenance(
        self, findings: list[FindingItem], registry: dict[str, Any]
    ) -> list[FindingItem]:
        """Iterate through findings and validate evidence_span_id existence in registry.

        If a finding references a span_id not in the registry, mark it as a hallucination risk.
        """
        if not registry:
            return findings
        ...
        for finding in findings:
            ...
            # Check raw_data for evidence_span_id as requested
            span_id = raw_data.get("evidence_span_id")

            if span_id and span_id not in registry:
                finding = finding.model_copy()
                finding.evidence_status = "evidence_gap"
                finding.hallucination_risk = True
                finding.manual_review_needed = True

                # Prepend [幻觉风险] to title
                if not finding.title.startswith("[幻觉风险]"):
                    finding.title = f"[幻觉风险] {finding.title}"

                # Add markers in raw_data
                new_raw = dict(finding.raw_data or {})
                new_raw["provenance_validation"] = "failed_hallucination_risk"
                new_raw["hallucination_risk"] = True
                finding.raw_data = new_raw
    ...
    def _check_critical_module_blocks(
        ...
        *,
        ...
        for module_name in enabled_modules:
            ...
            run_templates = [
                tid for tid in binding.hermes_templates if tid in run_results
            ]
