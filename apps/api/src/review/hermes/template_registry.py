...
class HermesTemplateRegistry:
    ...
    def load_templates(self) -> list[AgentTemplate]:
        ...
        for directory in directories:
            ...
            for path in sorted(directory.rglob("*.json")):
                try:
                    template = AgentTemplate.model_validate_json(
                        path.read_text(encoding="utf-8")
                    )
                    template.id = normalize_template_id(template.id) or template.id
                    template.metadata = {
                        **template.metadata,
                        "review_modules": template.metadata.get("review_modules")
                        or template_review_modules(template.id),
                    }
                    templates.append(template)
                except Exception as exc:
                    logger.warning(
                        "[hermes_template_registry] Failed to load %s: %s", path, exc
                    )
    ...
    def select_templates(
        self, *, brief: ReviewBrief, hermes_input: dict[str, Any]
    ) -> list[AgentTemplateMatch]:
        matches: list[AgentTemplateMatch] = []
        enabled = {
            normalize_template_id(item) or item
            for item in set(hermes_input.get("enabledAgents") or [])
        }
        disabled = {
            normalize_template_id(item) or item
            for item in set(hermes_input.get("disabledAgents") or [])
        }
        ...
        for template in self.load_templates():
            if template.id in disabled:
                continue
            if (
                template.supported_document_types
                and str(brief.review_object_type)
                not in template.supported_document_types
            ):
                continue
            score = 0
            reasons: list[str] = []
            if enabled:
                if template.id in enabled:
                    ...
                    continue
            if (
                not template.supported_document_types
                or str(brief.review_object_type) in template.supported_document_types
            ):
                score += 5
            ...
            if pack_ids and any(
                pack_id in pack_ids
                for pack_id in template.metadata.get("preferred_pack_ids", [])
            ):
                score += 3
            ...
            if score > 0:
                matches.append(
                    AgentTemplateMatch(template=template, score=score, reasons=reasons)
                )
    ...
    def focus_gaps(
        self,
        *,
        selected_templates: list[AgentTemplateMatch],
        brief: ReviewBrief,
        ...
        selected_keywords = {
            ...
            for keyword in match.template.focus_keywords
        }
        gaps: list[str] = []
        for term in self._focus_terms(brief, hermes_input):
            if term and not any(
                term == keyword or term in keyword or keyword in term
                for keyword in selected_keywords
            ):
                gaps.append(term)
        return list(dict.fromkeys(gaps))

    def save_runtime_template(
        self, template: AgentTemplate, *, task_id: str | None = None
    ) -> Path:
    ...
    def _focus_terms(
        ...
        if isinstance(raw_focus, str):
            terms = [
                part.strip()
                for part in raw_focus.replace("；", "，").split("，")
                if part.strip()
            ]
        else:
        ...
        terms.extend(
            str(tag).strip()
            for tag in ((brief.focus_pack or {}).get("discipline_tags") or [])
            if str(tag).strip()
        )
        if brief.query:
