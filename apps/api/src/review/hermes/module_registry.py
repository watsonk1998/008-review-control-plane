...
class HermesModuleRegistry:
    ...
    async def run_module(self, module_id: str, *, workspace: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        if module_id == 'parse_visibility':
            return self._run_parse_visibility(workspace, context)
        if module_id == 'fact_extract':
            return self._run_fact_extract(workspace, context)
        if module_id == 'profile_and_packs':
            return self._run_profile_and_packs(workspace, context)
        if module_id == 'rule_and_evidence':
            return self._run_rule_and_evidence(workspace, context)
        if module_id == 'structured_review_worker':
            return await self._run_structured_review_worker(workspace, context)
        raise KeyError(f'Unsupported Hermes module: {module_id}')

    def _run_parse_visibility(self, workspace: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
    ...
    def _run_fact_extract(self, workspace: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        parse_result = workspace.get('parse_result')
        if parse_result is None:
            self._run_parse_visibility(workspace, context)
    ...
    def _run_profile_and_packs(self, workspace: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        facts = workspace.get('facts')
        if facts is None:
            self._run_fact_extract(workspace, context)
    ...
    def _run_rule_and_evidence(self, workspace: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        parse_result = workspace.get('parse_result')
        if parse_result is None:
            self._run_parse_visibility(workspace, context)
            parse_result = workspace['parse_result']
        ...
        if executable_packs is None or resolved_profile is None:
            self._run_profile_and_packs(workspace, context)
    ...
    async def _run_structured_review_worker(self, workspace: dict[str, Any], context: dict[str, Any]) -> dict[str, Any]:
        ...
        packet.metadata = {
            **packet.metadata,
            'module_id': 'structured_review_worker',
