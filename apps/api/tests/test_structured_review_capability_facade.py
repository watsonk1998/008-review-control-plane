        ...
        return [
            {
                ...
                'applicabilityState': getattr(candidate, 'applicabilityState', 'applies'),
...
async def test_structured_review_capability_facade_primary_review_matches_executor(tmp_path: Path):
    sample = _write_sample(tmp_path)
    executor = StructuredReviewExecutor(document_loader=DocumentLoader(), llm_gateway=FakeLLM(), fast_adapter=None)
...
def test_hermes_module_registry_source_does_not_grab_executor_internals():
    source = (Path(__file__).resolve().parents[1] / 'src' / 'review' / 'hermes' / 'module_registry.py').read_text(encoding='utf-8')
    for forbidden in [
