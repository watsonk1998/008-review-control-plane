...
class TestHermesLocalKernelMinimalExecution(IsolatedAsyncioTestCase):
    ...
    def test_non_default_invariant(self):
        """Minimal execution MUST NOT be enabled by default."""
        fresh_adapter = HermesLocalKernelAdapter(launcher=self.launcher)
        self.assertFalse(fresh_adapter.available)
        self.assertFalse(fresh_adapter._is_enabled)
    ...
    async def test_successful_minimal_review(self, mock_exec):
        ...
        brief = ReviewBrief(review_id="rev-001", review_object_type="construction_scheme")
    ...
    async def test_execution_timeout(self, mock_exec):
        ...
        brief = ReviewBrief(review_id="rev-002", review_object_type="construction_scheme")
    ...
    async def test_invalid_json_output(self, mock_exec):
        ...
        mock_exec.return_value = MockProcess(stdout_str.encode('utf-8'), b"Traceback...")
        
        brief = ReviewBrief(review_id="rev-003", review_object_type="construction_scheme")
        packet = await self.adapter.review(brief)
        
        self.assertTrue(packet.degraded)
        self.assertEqual(packet.error, "local_kernel_error")
        self.assertIn("No <<<HERMES_KERNEL_RESULT>>> markers found", packet.metadata.get("error", ""))
