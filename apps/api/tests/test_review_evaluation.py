...
from src.review.evaluation.harness import (
    run_ablations,
    run_cross_model,
    run_cross_pack,
    run_main,
...
from src.review.evaluation.metrics import (
    _evidence_traceability,
    _review_preparation_provenance_consistency,
)
...
def _build_legacy_case(root: Path) -> None:
    ...
    _write_json(
        ...
        {"attachments": {"attachment-1": "attachment_unparsed"}},
    )
...
def test_review_eval_harness_reports_stage_metrics_and_versioned_diagnostics(
    ...
    monkeypatch.setattr(
        "src.review.evaluation.harness._passes_thresholds", lambda metrics: True
    )
    monkeypatch.setattr(
        "src.review.evaluation.harness._passes_versioned_stage_thresholds",
        lambda metrics: True,
    )
    ...
    assert (
        main_payload["layeredMetrics"]["L0"]["metrics"][
            "attachment_visibility_accuracy"
        ]
        >= 0
    )
    assert (
        main_payload["layeredMetrics"]["L0"]["metrics"]["preflight_gate_consistency"]
        >= 0
    )
    ...
    assert (
        main_payload["layeredMetrics"]["CrossCutting"]["metrics"][
    ...
    diagnostics = main_payload["versionedDiagnostics"]["cases"][0][
    ...
    assert (
        "official"
        in cross_model_payload["models"]["deterministic"]["byDocumentTypeReadiness"]
    )
    assert (
    ...
    replay_code, replay_payload = run_replay(
        tmp_path, case_ids=["versioned-hz-001"], output_dir=tmp_path / "replay-output"
    )
    ...
    assert replay_payload["selection"]["outputDirectory"] == str(
        (tmp_path / "replay-output").resolve()
    )
    replay_case = replay_payload["cases"][0]
    assert replay_case["artifactCapture"] is not None
    assert (
        "structured-review-result.json" in replay_case["artifactCapture"]["artifacts"]
    )
...
def test_review_preparation_provenance_consistency_distinguishes_versioned_vs_runtime():
    ...
    versioned_score, versioned_checks = _review_preparation_provenance_consistency(
        versioned_case, result
    )
    runtime_score, runtime_checks = _review_preparation_provenance_consistency(
        runtime_case, result
    )
