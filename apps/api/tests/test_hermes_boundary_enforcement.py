...
@pytest.fixture
def temp_dirs():
    with tempfile.TemporaryDirectory() as seed_dir, tempfile.TemporaryDirectory() as runtime_dir:
...
def test_online_main_chain_does_not_save_runtime_templates(temp_dirs):
    ...
    registry = HermesTemplateRegistry(seed_dir=seed_dir, runtime_dir=runtime_dir, formal_mode=True)
...
def test_basis_pack_resolver_closed_loop():
    ...
    resolver.profile_mapping = {
        "construction_org": {"profile_id": "test_profile", "rule_pack_ids": ["test_rule_pack"]}
    }
    resolver.rule_pack_registry = {
        "test_rule_pack": {"related_pack_ids": ["related_pack_1"]}
    }
    resolver.pack_registry = {
        "related_pack_1": {"basis_ids": ["basis_a", "basis_b"]}
    }
    ...
    profile = ResolvedReviewProfile(documentType="construction_org", requestedPolicyPackIds=[], requestedDisciplineTags=[])
...
def test_simulation_allows_runtime_template_save(temp_dirs):
    ...
    """
    seed_dir, runtime_dir = temp_dirs
    registry = HermesTemplateRegistry(seed_dir=seed_dir, runtime_dir=runtime_dir, formal_mode=False)
